...
def is_safe(input_list):
    """
    This function checks if the line is 'safe' or not and checks if the line is safe by removing a level.
    input_list: This is a list of integers
    output: True if the line is safe and false otherwise."""
    if (check_increasing(input_list) | check_decreasing(input_list)) & check_deltas(input_list):
        return True
    for ind in range(len(input_list)):
        temp_list = input_list.copy()
        _ = temp_list.pop(ind)
        if (check_increasing(temp_list) | check_decreasing(temp_list)) & check_deltas(temp_list):
            return True
    return False
